...
@patch(f"{PKG}.{KERNEL_CLASS}.loss_is_decreasing")
def test_aggressive_momentum_first_kernel_step(
    mock_loss_decreasing: Mock,
    ...
    mock_loss_decreasing.assert_not_called()
...
@patch(f"{PKG}.compute_loss")
def test_aggressive_momentum_loss_is_decreasing_returns_true_when_current_is_lower(
...
@patch(f"{PKG}.compute_loss")
def test_aggressive_momentum_loss_is_decreasing_returns_true_when_current_is_higher(
    mock_compute_loss: Mock, fixture_with_tol: Fixture
) -> None:
    (_, _, kernel) = fixture_with_tol
...
@patch(f"{PKG}.compute_loss")
def test_aggressive_momentum_loss_is_decreasing_returns_true_when_current_is_equal(
    mock_compute_loss: Mock, fixture_with_tol: Fixture
) -> None:
    (_, _, kernel) = fixture_with_tol
    kernel.parameter_update_loss = 1.0  # previous loss
