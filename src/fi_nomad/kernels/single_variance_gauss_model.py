...
class SingleVarianceGaussianModelKernel(KernelBase):
    """Estimator for a Gaussian model (L, v) for a sparse nonnegative matrix X with shared
    variance across all entries.

    The algorithm uses an expectation-maximization strategy to learn the parameters of a
    Gaussian model with means L and universal variance v that maximizes the likelihood of
    the data X.
    ...
    def __init__(self, indata: KernelInputType) -> None:
        ...
        initial_variance = float(np.var(indata.sparse_matrix_X))
    ...
    def step(self) -> None:
        """Each step is comprised of three parts: posterior-evaluation, L-update, and
        likelihood monitoring.

        Posterior evaluation computes the means and elementwise variance of the current
        estimate, given the original data.

        L-update enforces low rank on the current posterior means, estimates the variance
        of that new model, and computes gamma (the means normalized by the model standard
        deviation).
        ...
        if likelihood < self.likelihood:
            logger.warning(
                f"Iteration ${self.elapsed_iterations}: Likelihood decreased, "
                + f"from {self.likelihood} to {likelihood}"
            )
    ...
    def running_report(self) -> str:
        txt = (
            f"\t\tIteration {self.elapsed_iterations}: loss: {self.loss} "
    ...
    def report(self) -> KernelReturnType:
        text = (
            f"{self.elapsed_iterations} total iterations, final loss "
